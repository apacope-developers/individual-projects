  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card bg-gray-900 relative overflow-hidden min-h-[240px]">
      ...
      <div className="card mt-4">
        <div className="flex items-center justify-center space-x-4">
          {!inCall ? (
            <button onClick={startCall} className="btn-primary flex items-center px-8 py-3">
              <Video className="w-5 h-5 mr-2" />
              {t('startCall')}
            </button>
          ) : (
            <>
              <button onClick={toggleAudio} className={`p-4 rounded-full ${isAudioEnabled ? 'bg-gray-200' : 'bg-red-500 text-white'}`}>
                {isAudioEnabled ? <Mic className="w-6 h-6" /> : <MicOff className="w-6 h-6" />}
              </button>
              <button onClick={toggleVideo} className={`p-4 rounded-full ${isVideoEnabled ? 'bg-gray-200' : 'bg-red-500 text-white'}`}>
                {isVideoEnabled ? <Video className="w-6 h-6" /> : <VideoOff className="w-6 h-6" />}
              </button>
              <button onClick={endCall} className="p-4 rounded-full bg-red-500 text-white">
