...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .btn-ghost{background:transparent;color:#A1A1AA;border:1px solid #27272A}.btn-ghost:hover{background:#18181B;border-color:#3F3F46;color:#FAFAFA}
...
<div class="flex min-h-screen">
    ...
    <aside class="admin-sidebar w-56 bg-black/50 border-r border-zinc-800/50 flex flex-col py-5 flex-shrink-0 fixed h-full z-10">
        <div class="px-4 mb-6 flex items-center gap-3">
            ...
            <div>
                <h1 class="font-bold text-sm leading-tight">LifeLine</h1>
                <p class="text-[10px] text-zinc-500">Admin Panel</p>
            </div>
        </div>
        <nav class="flex-1 flex flex-col gap-1 px-2">
            <a href="{{ route('admin') }}" class="sidebar-link {{ request()->routeIs('admin') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.body-map') }}" class="sidebar-link {{ request()->routeIs('admin.body-map*') ? 'active' : '' }}">
                <i class="fa-solid fa-person w-5 text-center"></i>
                <span>Body Map</span>
            </a>
            <a href="{{ route('admin.symptom-checker') }}" class="sidebar-link {{ request()->routeIs('admin.symptom-checker*') ? 'active' : '' }}">
                <i class="fa-solid fa-stethoscope w-5 text-center"></i>
                <span>Symptom Checker</span>
            </a>
            <a href="{{ route('admin.first-aid-guide') }}" class="sidebar-link {{ request()->routeIs('admin.first-aid-guide*') ? 'active' : '' }}">
                <i class="fa-solid fa-book-medical w-5 text-center"></i>
                <span>First Aid Guide</span>
            </a>
            <a href="{{ route('admin.kit-inventory') }}" class="sidebar-link {{ request()->routeIs('admin.kit-inventory*') ? 'active' : '' }}">
                <i class="fa-solid fa-kit-medical w-5 text-center"></i>
                <span>Kit Inventory</span>
            </a>
            <a href="{{ route('admin.contacts') }}" class="sidebar-link {{ request()->routeIs('admin.contacts*') ? 'active' : '' }}">
                <i class="fa-solid fa-phone-volume w-5 text-center"></i>
